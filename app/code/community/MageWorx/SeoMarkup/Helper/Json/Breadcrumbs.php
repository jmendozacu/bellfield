...
class MageWorx_SeoMarkup_Helper_Json_Breadcrumbs extends MageWorx_SeoMarkup_Helper_Abstract
{

    public function getJsonBreadcrumbsData($breadcrumbs)
    {
        if (!$this->_helperConfig->isBreadcrumbsRichsnippetEnabled()) {
            return false;
        }

        if (!$this->_helperConfig->isBreadcrumbsJsonLdMethod()) {
            return false;
        }
    ...
    protected function _getBreadcrumbs($breadcrumbs)
    {
        ...
        if (!empty($cacheKeyInfo['crumbs'])) {
            ...
            } elseif($this->_helper->isCategoryPage()) {
                $crumbs = $this->_getCatalogBreadcrumbs($crumbs);
            } elseif($this->_helper->isCmsPage()) {
                $crumbs = $this->_getCmsPageBreadcrumbs($crumbs);
            } elseif($this->_helper->isHomePage()) {
