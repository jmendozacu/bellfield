...
class MageWorx_SeoReports_Adminhtml_Mageworx_Seoreports_CategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        if (!array_key_exists('store', $this->getRequest()->getParams())) {
            return $this->_redirect('*/*/*', array('store' => Mage::helper('mageworx_seoreports')->getDefaultStoreId()));
        }

        if (!Mage::helper('mageworx_seoreports')->getCategoryReportStatus()) {
            Mage::getSingleton('adminhtml/session')->addWarning(
                Mage::helper('mageworx_seoreports')
    ...
    public function runGenerateAction()
    {
        ...
        switch ($action) {
            case 'start':
                ...
                $connection->truncate($this->_getReportTableName());
                $action         = 'preparation';
            case 'preparation':
                ...
                $action         = 'calculation';
                $result['text'] = $this->__(
                    'Total %1$s, processed %2$s records (%3$s%%)...', $total, $current,
            ...
            case 'calculation':
                ...
                if (($current < $total)) {
                    ...
                    if ($current >= $total) {
                        $current        = $total;
                        $result['stop'] = 1;
                        Mage::helper('mageworx_seoreports')->setCategoryReportStatus(1);
                        $this->_getSession()->addSuccess(Mage::helper('mageworx_seoreports')->__('Report has been successfully generated.'));
                        break;
                    }
                    ...
                    if ($current <= $limit) {
                        ...
                        $result['text'] = $this->__(
                            'Total %1$s, processed %2$s records (%3$s%%)...', $total - 1,
    ...
    {
        $attributes = array('name', 'url_path', 'meta_title', 'meta_description');
    ...
    protected function _prepareCategories()
    {
        ...
        foreach ($stores as $storeId) {
            ...
            foreach ($this->_categories as $category) {
                ...
                $connection->insert($this->_getReportTableName(), $this->_getPreparedData($item, $storeId));
            }
    ...
    {
        if (Mage::helper('mageworx_seoreports')->_prepareText($model->getMetaTitle())) {
            return Mage::helper('mageworx_seoreports')->_prepareText($model->getMetaTitle());
        }
    ...
    /**
     *
     * @param Magentp_Catalog_Model_Category $item
     * @param int $storeId
     * @return array
     */
    protected function _getPreparedData($item, $storeId)
    {
        /** @var MageWorx_SeoReports_Helper_Data $helper */
        $helper = Mage::helper('mageworx_seoreports');
    ...
    {
        $connection  = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tableName = $this->_getReportTableName();

        $sql = "UPDATE `" . $tableName . "` AS srp,
                    (SELECT `prepared_name`, `store_id`, COUNT(*) AS dupl_count FROM `" . $tableName . "` WHERE `store_id`=" . intval($storeId) . " AND `prepared_name`!='' GROUP BY `prepared_name`) AS srpr
        ...
        $sql = "UPDATE `" . $tableName . "` AS srp,
                    (SELECT `prepared_meta_title`, `store_id`, COUNT(*) AS dupl_count FROM `" . $tableName . "` WHERE `store_id`=" . intval($storeId) . " AND `prepared_meta_title`!='' GROUP BY `prepared_meta_title`) AS srpr
        ...
        $sql = "UPDATE `" . $tableName . "` AS srp,
                    (SELECT `url`, `store_id`, COUNT(*) AS dupl_count FROM `" . $tableName . "` WHERE `store_id`=" . intval($storeId) . " AND `url`!='' GROUP BY `url`) AS srpr
    ...
    public function duplicateViewAction()
    {
        if (!Mage::helper('mageworx_seoreports')->getCategoryReportStatus()){
             Mage::getSingleton('adminhtml/session')
                    ->addWarning(Mage::helper('mageworx_seoreports')->__('You need to generate the report due to recent changes.'));
        }
     ...
     * @return string
     */
    protected function _getReportTableName()
    {
        /** @var MageWorx_SeoReports_Model_Resource_Category $resource */
        $resource = Mage::getResourceSingleton('mageworx_seoreports/category');
