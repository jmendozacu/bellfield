...
class MageWorx_SeoReports_Adminhtml_Mageworx_Seoreports_CmsController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        ...
        if (!Mage::helper('mageworx_seoreports')->getCmsReportStatus()){
            ...
            case 'calculation':
                ...
                if ($current < $total) {
                    ...
                    if ($current >= $total) {
                        $current        = $total;
                        $result['stop'] = 1;
                        Mage::helper('mageworx_seoreports')->setCmsReportStatus(1);
    ...
    protected function _prepareCms()
    {
        ...
        foreach ($stores as $storeId) {
            ...
            foreach ($collection as $item) {
                ...
                $connection->insert(
                    $tablePrefix . 'seosuite_report_cms', array(
                    'page_id'             => $item->getId(),
                    'store_id'            => $storeId,
    ...
    {
        if (!Mage::helper('mageworx_seoreports')->getCmsReportStatus()){
            Mage::getSingleton('adminhtml/session')->addWarning(Mage::helper('mageworx_seoreports')->__('You need to generate the report due to recent changes.'));
        }
