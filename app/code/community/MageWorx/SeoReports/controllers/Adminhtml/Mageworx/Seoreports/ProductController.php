...
class MageWorx_SeoReports_Adminhtml_Mageworx_Seoreports_ProductController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        ...
        if (!Mage::helper('mageworx_seoreports')->getProductReportStatus()) {
            ...
            case 'calculation':
                ...
                if ($current < $total) {
                    ...
                    if ($current >= $total) {
                        $current        = $total;
                        $result['stop'] = 1;
                        Mage::helper('mageworx_seoreports')->setProductReportStatus(1);
    ...
    protected function _getProductCollection($store, $from = 0, $limit = 900000000)
    {
        ...
        if ($store) {
            $collection->joinAttribute(
                'custom_name', 'catalog_product/name', 'entity_id', null, 'inner',
                $store->getId()
            );
        }
    ...
    {
        /** @var MageWorx_SeoReports_Model_Resource_Product $resource */
        $resource = Mage::getResourceSingleton('mageworx_seoreports/product');
