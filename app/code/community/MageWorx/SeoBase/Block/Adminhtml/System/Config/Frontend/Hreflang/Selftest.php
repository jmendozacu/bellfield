...
class MageWorx_SeoBase_Block_Adminhtml_System_Config_Frontend_Hreflang_Selftest extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getSource()
    {
        ...
        foreach ($allStores as $store) {
            ...
            $alternateProductCodes = Mage::helper('mageworx_seobase/hreflang')->getAlternateRawCodes(
                'product',
                $store->getStoreId()
            );

            if (!empty($alternateProductCodes[$store->getStoreId()])) {
                $duplicateProduct = $this->_markDuplicateData(
                    $data, 'product_hreflang_code', $store->getWebsiteId(),
            ...
            $alternateCategoryCodes = Mage::helper('mageworx_seobase/hreflang')->getAlternateRawCodes(
                'category',
                $store->getStoreId()
            );

            if (!empty($alternateCategoryCodes[$store->getStoreId()])) {
                $duplicateCategory = $this->_markDuplicateData(
                    $data, 'category_hreflang_code', $store->getWebsiteId(),
            ...
            if (!empty($alternateCmsCodes[$store->getStoreId()])) {
                $duplicateCms = $this->_markDuplicateData(
                    $data, 'cms_hreflang_code', $store->getWebsiteId(),
