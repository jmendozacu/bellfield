...
class MageWorx_SeoBase_Block_Rss_List extends Mage_Rss_Block_List
{
    ...
    public function NewProductRssFeed()
    {
        ...
        if ((bool) Mage::getStoreConfig($path)) {
            $this->__addRssFeed(
                Mage::getUrl('rss/' . Mage::app()->getStore()->getCode() . '/@new'),
    ...
    public function SpecialProductRssFeed()
    {
        ...
        if ((bool) Mage::getStoreConfig($path)) {
            $this->__addRssFeed(
                Mage::getUrl('rss/' . Mage::app()->getStore()->getCode() . '/@specials/' . $this->getCurrentCustomerGroupId()),
    ...
    public function SalesRuleProductRssFeed()
    {
        ...
        if ((bool) Mage::getStoreConfig($path)) {
            $this->__addRssFeed(
                Mage::getUrl('rss/' . Mage::app()->getStore()->getCode() . '/@discounts/' . $this->getCurrentCustomerGroupId()),
    ...
    public function CategoriesRssFeed()
    {
        ...
        if ((bool) Mage::getStoreConfig($path)) {
            ...
            foreach ($collection as $category) {
                $this->__addRssFeed(
                    Mage::getUrl('rss/' . Mage::app()->getStore()->getCode() . '/' . $category->getUrlKey()),
                    $category->getName()
                );
            }
        }
