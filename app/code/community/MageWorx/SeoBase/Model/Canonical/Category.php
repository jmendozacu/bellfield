...
class MageWorx_SeoBase_Model_Canonical_Category extends MageWorx_SeoBase_Model_Canonical_Abstract
{
    ...
    protected function _getCanonicalUrl($item = null, $storeId = null)
    {
        ...
        $url = $this->_deleteByGetParamList($storeId, $url);
    ...
    protected function _changePagerParameterToCurrentForCurrentUrl()
    {
        ...
        if (is_object($pager)) {
            ...
            elseif ($pageNum == 1 && $this->_helperLF->isLNFriendlyUrlsEnabled() && $this->_helperData->getPagerUrlFormat()) {
                return $this->_deletePagerParameter(
     ...
     * @return string
     */
    public function deleteParametrsFromUrl($url, array $cropParams, $reverse = false)
    {
        $parseUrl = parse_url($url);
        ...
        if ($reverse) {
            $params = array_intersect_key($params, array_flip($cropParams));
        } else {
     ...
     * @return string
     */
    protected function _deleteByGetParamList($storeId, $url)
    {
        /** @var MageWorx_SeoBase_Helper_Data $helper */
        $helper = Mage::helper('mageworx_seobase');
        $params = $helper->getCanonicalParams($storeId);

        if (!$params) {
            return $url;
        }

        $listType = $helper->getCanonicalParamsListType($storeId);
        $revers   = ($listType == MageWorx_SeoBase_Model_System_Config_Source_ParamListType::WHITE_LIST_ID);

        $url = $this->deleteParametrsFromUrl($url, $params, $revers);
