...
class MageWorx_XSitemap_Model_Sitemap extends Mage_Core_Model_Abstract
{
    ...
    protected function _construct()
    {
        ...
        $this->_generator = Mage::getModel('mageworx_xsitemap/generator');
    }
    ...
    protected function _beforeSave()
    {
        ...
        if (!$this->_io->allowedPath($realPath, Mage::getBaseDir())) {
            Mage::throwException(Mage::helper('mageworx_xsitemap')->__('Please define correct path'));
        }
        ...
        if (!preg_match('#^[a-zA-Z0-9_\.]+$#', $this->getSitemapFilename())) {
            Mage::throwException(Mage::helper('mageworx_xsitemap')->__('Please use only letters (a-z or A-Z), numbers (0-9) or underscore (_) in the filename. No spaces or other characters are allowed.'));
        }
    ...
    protected function _checkFileNameIsOriginal()
    {
        ...
        if ($alreadyIs) {
            ...
            Mage::throwException(Mage::helper('mageworx_xsitemap')->__($mes, $otherFilePath[$alreadyIs], $alreadyIs));
        }
    }
    ...
    protected function _getOtherModelFullPathFilenames()
    {
        ...
        foreach ($models as $model) {
            $filepaths[$model->getId()] = $this->getFullPathFilename(
    ...
    public function generateXml($entity = false, $currentInc = false)
    {
        $this->_generator->generateXml($this, $entity, $currentInc);
