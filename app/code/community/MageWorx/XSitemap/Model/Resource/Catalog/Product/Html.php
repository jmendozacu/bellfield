...
class MageWorx_XSitemap_Model_Resource_Catalog_Product_Html extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
            ...
            if ($attribute['is_global']) {
                ...
                $this->_select->joinLeft(
                    ...
                    $this->_getWriteAdapter()->quoteInto(
                        't1_' . $attributeCode . '.entity_id = t2_' . $attributeCode . '.entity_id AND t1_' . $attributeCode . '.attribute_id = t2_' . $attributeCode . '.attribute_id AND t2_' . $attributeCode . '.store_id=?',
                        $storeId
                    ), array()
                )
                    ->where(
                        'IFNULL(t2_' . $attributeCode . '.value, t1_' . $attributeCode . '.value)' . $conditionRule,
                        $value
                    );
            }
        }
    ...
    public function getCollection($catId)
    {
        ...
        $productUrlToCategory = Mage::helper('mageworx_xsitemap')->getHtmlSitemapProductUrlType();
        $onlyCount            = false;
        ...
        if ($name) {
            ...
            $this->_select->joinRight(
                ...
                "cpev.attribute_id = ea.attribute_id" .  new Zend_Db_Expr(
                    " AND cpev.store_id =
                    IF(
        ...
        if ($excludeAttr) {
            $this->_select->joinLeft(
                ...
                'exclude_tbl.entity_id = e.entity_id AND exclude_tbl.attribute_id = ' . $excludeAttr->getAttributeId() . new Zend_Db_Expr(
                    " AND exclude_tbl.store_id =
                    IF(
        ...
        if(Mage::helper('mageworx_xsitemap')->isExcludeFromHTMLOutOfStockProduct($storeId)){
        ...
        if ($productUrlToCategory == 'canonical') {
            ...
            $productCanonicalType = Mage::helper('mageworx_xsitemap/adapter_seobase')->getSeoBaseProductCanonicalType();
        ...
        if ($productUrlToCategory == 'no') {
            if (!Mage::helper('mageworx_xsitemap')->isProductUrlUseCategory()) {
                $sort         = '';
                $cropCategory = true;
            }
        }
        ...
        if (Mage::helper('mageworx_seoall/version')->isEeRewriteActive()) {
            ...
            $this->_select
                ...
                ->joinLeft(
                    array('euur' => $this->getTable('enterprise_urlrewrite/url_rewrite')),
                    'ecp.url_rewrite_id = euur.url_rewrite_id AND euur.is_system = 1',
                    array()
                )
                ->joinLeft(
                    array('ecp2' => $this->getTable('enterprise_catalog/product')),
                    'ecp2.product_id = e.entity_id AND ecp2.store_id = 0',
                    array()
                )
                ->joinLeft(
                    array('euur2' => $this->getTable('enterprise_urlrewrite/url_rewrite')),
        ...
        $sortOrder = Mage::helper('mageworx_xsitemap')->getHtmlSitemapSort();
    ...
    protected function _getProductDataByCategory($catId)
    {
        $collection = Mage::getModel('catalog/product')
            ->getCollection()
            ->joinField(
                'category_id', $this->_resources->getTableName('catalog/category_product'), 'category_id',
