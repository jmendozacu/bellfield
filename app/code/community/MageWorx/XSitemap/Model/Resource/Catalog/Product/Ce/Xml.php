...
class MageWorx_XSitemap_Model_Resource_Catalog_Product_Ce_Xml extends MageWorx_XSitemap_Model_Resource_Catalog_Product_AbstractXml
{
    ...
    public function getCollection($storeId, $onlyCount = false, $limit = 4000000000, $from = 0)
    {
        ...
        if(Mage::helper('mageworx_xsitemap')->isExcludeFromXMLOutOfStockProduct($storeId)){
        ...
        if (Mage::helper('mageworx_xsitemap/adapter_seobase')->isSeoBaseAvailable() &&
            Mage::helper('mageworx_xsitemap/adapter_seobase')->isSeoBaseCanonicalUrlEnabled($storeId) &&
            Mage::helper('mageworx_xsitemap/adapter_seobase')->getSeoBaseProductCanonicalType($storeId))
        {
            $productCanonicalType = Mage::helper('mageworx_xsitemap/adapter_seobase')->getSeoBaseProductCanonicalType($storeId);
            ...
            $length = Mage::helper('mageworx_xsitemap')->getXmlSitemapUrlLength();
            if ($length == 'short') {
                $sort = 'ASC';
            }
            elseif ($length == 'long') {
        ...
        $isAddImage = Mage::helper('mageworx_xsitemap')->isProductImages();
        ...
        if ($isAddImage) {
            ...
            if ($mediaAttr) {
                $this->_select->joinLeft(
                    ...
                    'media_tbl.entity_id = e.entity_id AND media_tbl.attribute_id = ' . $mediaAttr->getAttributeId() . new Zend_Db_Expr(
                        " AND media_tbl.store_id =
                    IF(
                        ((SELECT `exclude`.`value` FROM `{$mediaAttr->getBackend()->getTable()}` AS `exclude` 
                            WHERE `exclude`.`entity_id` = `e`.`entity_id` AND `attribute_id` = {$mediaAttr->getAttributeId()} AND `store_id` = $storeId) IS NOT NULL),
                          (SELECT $storeId),
                          (SELECT 0)
                          )"
                    ),
