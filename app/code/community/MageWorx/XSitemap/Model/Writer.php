...
class MageWorx_XSitemap_Model_Writer
{
    ...
    {
        $hasImagesLink = Mage::helper('mageworx_xsitemap')->isProductImages();
        if (isset($hasImagesLink) and $hasImagesLink !== '') {
            $this->_hasImagesLink = $hasImagesLink;
        }

        $hasAlternateLink = Mage::helper('mageworx_xsitemap')->isAlternateUrlsEnabled();
        if (isset($hasAlternateLink) and $hasAlternateLink !== '') {
            $this->_hasAlternateLink = $hasAlternateLink;
        }

        $this->_useCssForXmlSitemap = Mage::helper('mageworx_xsitemap')->isUseCssForXmlSitemap();

        $useIndex = Mage::helper('mageworx_xsitemap')->useIndex();
        if (isset($useIndex) and $useIndex !== '') {
            $this->_useIndex = $useIndex;
        }

        $splitSize = Mage::helper('mageworx_xsitemap')->getSplitSize();
        if (isset($splitSize) and $splitSize !== '') {
            $this->_splitSize = ($splitSize < 20480) ? $this->_splitSize = 20480 : $splitSize;
        }

        $maxLinks = Mage::helper('mageworx_xsitemap')->getMaxLinks();
...
//        return (!$result) ? false : true;
    ...
    protected function _isInputDataValid($url, $lastmod, $changefreq, $priority)
    {
        ...
        if ($this->_changefreqValidator->isValid($changefreq) == false) {
            throw new Exception(
                $this->_helper->__(
                    "Changefreq value '%s' is not valid. Item url: '%s'.", $changefreq,
                    $url
                )
            );
        }
        ...
        if ($this->_priorityValidator->isValid($priority) == false) {
            throw new Exception(
                $this->_helper->__(
                    "Priority value '%s' is not valid. Item url: '%s'.", $priority,
                    $url
                )
            );
        }
    ...
    protected function _generateSitemapIndex()
    {
        ...
        if (!$this->_isXmlFileValid($this->_fileName)) {
            throw new Exception(
                $this->_helper->__(
                    "Sitemap index xml file '%s' isn\'t valid.",
