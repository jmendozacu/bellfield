...
class MageWorx_XSitemap_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getHtmlSitemapProductUrlType()
    {
        ...
        elseif ($param == 'canonical') {
            $seoHelper = Mage::helper('mageworx_xsitemap/adapter_seobase')->getSeoBaseHelper('isCanonicalUrlEnabled');
            if ($seoHelper) {
    ...
    public function getAlternateFinalCodes($type, $storeId)
    {
        if($this->isAlternateUrlsEnabled()){
            return Mage::helper('mageworx_xsitemap/adapter_seobase')->getSeoAlternateFinalCodes($type, $storeId);
        }
    ...
    public function isHomePage($url)
    {
        ...
        if (strpos($homeIdentifier, '|') !== false && strpos($homeIdentifier, '|') !== 0) {
