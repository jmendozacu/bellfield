...
abstract class MageWorx_SeoXTemplates_Controller_Adminhtml_Seoxtemplates_Template extends Mage_Adminhtml_Controller_Action
{
    ...
    public function runApplyAction()
    {
        ...
        elseif ($test == 'start') {
            ...
            $result['redirect'] = $this->getUrl('*/*/csv/', array('_secure' => true, 'file_name' => $name));
        }
        else {
    ...
    protected function _apply($nestedStoreId = null, $nextIds = null)
    {
        ...
        if ($current <= $total) {
            ...
            $this->_getHelper()->getTemplateAdapterByModel($this->_model)->apply(
                $itemCollection, $this->_model, $nestedStoreId,
                $testMode
            );
            $current += $limit;
            ...
            if ($current >= $total) {
                ...
                if (!empty($nestedStoreId) && !Mage::helper('mageworx_seoxtemplates/store')->isLastStoreId($nestedStoreId)) {
                    ...
                    if ($current == 0 && $total == 0) {
                        $result['text'] = $this->__(
                            'No products/categories from \'%s\' store view matching the conditions found. Preparing to generate data for the \'%s\' store view...',
                        ...
                        $result['text'] = $this->__(
                            'Generation for \'%s\' store view has completed... Preparing to generate data for the \'%s\' store view...',
                            Mage::helper('mageworx_seoxtemplates/store')->getStoreById($lastNestedStoreId)->getName(),
                            Mage::helper('mageworx_seoxtemplates/store')->getStoreById($nestedStoreId)->getName()
                        );
                    }
                    ...
                    if (!empty($nestedStoreId)) {
                        $result['text'] = $this->__(
                            'Generation for \'%s\' store view has completed... ',
                            Mage::helper('mageworx_seoxtemplates/store')->getStoreById($nestedStoreId)->getName()
                        );
                    }
                    else {
                ...
                if (intval($this->getRequest()->getParam('current', 0)) == 0) {
                    if (!empty($nestedStoreId)) {
                        $result['text'] = $this->__(
                            'Starting generation \'%s\' store view: ',
                            Mage::helper('mageworx_seoxtemplates/store')->getStoreById($nestedStoreId)->getName()
                        );
                    }
                    else {
    ...
    protected function _reindex($reindex, $nextIds)
    {
        ...
        elseif ($reindex == 'run') {
            ...
            if (!$index->getId()) {
                ...
                $result['redirect'] = $this->getUrl('*/*/index/', array('_secure' => true));
            }
            else {
                try {
                    $index->reindexAll();
    ...
    /**
     * Unset Unavailable Ids
     */
    public function unsetUnavailableIds($itemIds, $unavailableIds)
    {
        $itemIdsRevers = array_reverse($itemIds);
        foreach($unavailableIds as $id) {
            unset($id, $itemIdsRevers);
        }

        return array_reverse($itemIdsRevers);
    }
