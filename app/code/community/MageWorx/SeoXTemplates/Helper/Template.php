...
abstract class MageWorx_SeoXTemplates_Helper_Template extends Mage_Core_Helper_Abstract
{
    ...
    /**
     * Return assigned category ids
     *
     * @param int $id
     * @param string $type
     * @return array
     */
    public function getTemplateItems($id, $type)
    {
        /** @var MageWorx_SeoXTemplates_Model_Resource_Template_Collection $collection */
        $collection = Mage::getResourceModel('mageworx_seoxtemplates/template_' . $type . '_collection');
        $collection->loadByIds($id)->load();
        $template = $collection->getFirstItem();

        return $template->getIndividualRelatedModel()->getResource()->getItemIds($id);
    }
