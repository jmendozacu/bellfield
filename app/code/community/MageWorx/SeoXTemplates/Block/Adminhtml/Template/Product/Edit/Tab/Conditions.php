...
class MageWorx_SeoXTemplates_Block_Adminhtml_Template_Product_Edit_Tab_Conditions extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = Mage::getResourceModel('mageworx_seoxtemplates/catalog_product_collection')
            ...
            ->joinField(
                'qty', 'cataloginventory/stock_item', 'qty', 'product_id=entity_id', '{{table}}.stock_id=1',
                'left'
            );
    ...
    protected function _toHtml()
    {
        ...
        if ($useTemplateChooser) {
            $switcher .= '<label><input type="radio" name="general[assign_type]" value="4"' . ' onclick="assignBy(this.value)" /> '
                . Mage::helper('mageworx_seoxtemplates')->__('Use Product(s) from other template') .
                '</label>';
        }
    ...
    {
        $collection = Mage::getResourceModel("mageworx_seoxtemplates/template_product_collection")
    ...
    {
        $hint = '<div class="note"><p>' . $this->__('You can only choose among the templates with the "Specific Products" assigned Type.') . '</div>';
            ...
            '<div id="templateChooser" class="fieldset ">
                ...
                <div class="hor-scroll">
                    <table class="form-list" cellspacing="0">
                        <tbody>
                            <tr>
                                <td class="label"><label for="template_id">' . Mage::helper('catalog')->__('Use Product(s) from template') . '</label></td>
                                <td class="value">
                                    <select id="template_id" name="post_template_id" title="' . Mage::helper('catalog')->__('Use Product(s) from template') . '" class=" select">';
     ...
     * @param MageWorx_SeoXTemplates_Model_Resource_Template_Product_Collection $duplicate|null
     * @return array
     */
    protected function _getHtmlAttributes($duplicate)
    {
        ...
        if (Mage::helper('mageworx_seoxtemplates/factory')->getHelper()->isAssignForAllItems($template->getAssignType())) {
            ...
            $data['checkedGroup']      = '';
            $data['hideIndividualJS']  = Mage::helper('mageworx_seoxtemplates')->getHideElementJsString(array('templateProductGrid', 'templateProductAttributeSet', 'templateChooser'));
        }
        elseif (Mage::helper('mageworx_seoxtemplates/factory')->getHelper()->isAssignForIndividualItems($template->getAssignType())) {
            ...
            $data['checkedGroup']      = '';
            $data['hideIndividualJS']  = Mage::helper('mageworx_seoxtemplates')->getHideElementJsString(array('templateProductAttributeSet', 'templateChooser'));
        }
        elseif (Mage::helper('mageworx_seoxtemplates/factory')->getHelper()->isAssignForGroupItems($template->getAssignType())) {
            ...
            $data['hideIndividualJS']  = Mage::helper('mageworx_seoxtemplates')->getHideElementJsString(array('templateProductGrid', 'templateChooser'));
        }
        else {
            $data['checkedAll']        = '';
            $data['checkedIndividual'] = ' checked';
        ...
        if ($duplicate) {
            ...
            $data['title']    = ' title="' . Mage::helper('mageworx_seoxtemplates')->__(
                'You have already created a similar template (%s), which assigned for all product/categories of the templates store(s)',
                $duplicate->getName()
            ) . '"';
        }
        else {
