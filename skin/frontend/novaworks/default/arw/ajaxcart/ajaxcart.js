...
ajaxCart.prototype = {
                        ...
                        if (response.error) {
                            ...
                            if(response.add_to_cart == '1'){
                                if(jQuery('body').hasClass('arexworks-quickview-index')){
                                    ...
                                    _w_width = parseInt(jQuery(document).width());
									if(_w_width > 767){
										if(typeof hack_element_fly !== typeof undefined && hack_element_fly !== false && hack_element_fly){
										var hack_element_fly_id = jQuery(hack_element_fly).attr('id');
											if(typeof hack_element_fly_id !== typeof undefined && hack_element_fly_id !== false){
												console.log('yes 1');
												if(jQuery(hack_element_fly).attr('id') == 'product-addtocart-button'){
													arw_show_image_fly_when_click_addcart(jQuery(hack_element_fly), true);
												}else{
													arw_show_image_fly_when_click_addcart(jQuery(hack_element_fly), false);
												}
											}else{
												arw_show_image_fly_when_click_addcart(jQuery(hack_element_fly), false);
											}
										...
										jQuery('html,body').animate({scrollTop: jQuery(".header-mini-cart").offset().top}, 'slow');
									}
    ...
    updateMinicart: function() {
		...
		new Ajax.Request(url, {
                method: 'post',
                onSuccess: function(transport) {
					jQuery('#arw_mini_cart_header').html(transport.responseText);
					jQuery('#mobile_cart_summary').html(transport.responseText);
...
function arw_show_image_fly_when_click_addcart($button, is_product_page){
    if($button.length > 0){
        var cart = jQuery('#mini_cart_header_icon'),//jQuery('.mini-cart-header'),
            pass = true,
		...
		if(is_product_page){currentImg = jQuery('#product_main_image');}
		if(jQuery('.arexworks-quickview-index').length>0){pass = false; }
        if (currentImg && pass) {
            ...
            imgclone.animate({'width':0, 'height':0, 'top':cart.offset().top + 10, 'left':cart.offset().left + 10});
        }
    }
};