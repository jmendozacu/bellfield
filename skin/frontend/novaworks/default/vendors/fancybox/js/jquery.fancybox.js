(function (window, document, $, undefined) {
			...
			if (!type && obj.isDom) {
				...
				if (isString(href)) {
					if (!type) {
						...
						} else if (isString(element)) {type    = 'html';
							content = element;
						}
					}
					if (type === 'ajax') {
						hrefParts = href.split(/\s+/, 2);href      = hrefParts.shift();
	...
	F.transitions = {
		getOrigPosition: function () {
			...
			if (isQuery(orig)) {
				pos = orig.offset();
				if (orig.is('img')) {width  = orig.outerWidth();height = orig.outerHeight();}
			} else {
	...
	F.helpers.overlay = {
		...
		open : function(opts) {
			var that = this;
			opts = $.extend({}, this.defaults, opts);
			if (this.overlay) {this.overlay.unbind('.overlay').width('auto').height('auto');} else {
			this.create(opts);	}
