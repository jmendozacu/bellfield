LayNav = {
	...
	onSuccessSend : function(transport) {
		...
		var categoryProducts = $$('.category-products')[0];
		if (categoryProducts) Effect.ScrollTo(categoryProducts, { duration:'0.3', offset:-20 });
...
LayNav.PriceRange = {
	init : function(config) {
		...
		Event.observe(document.body, 'touchmove', function(ev){obj.touchHelper(ev)});
		Event.observe(document.body, 'touchend', function(ev){obj.touchHelper(ev)});
		Event.observe(this.getLeftPointer().up('dl'), 'touchstart', function(ev){obj.touchHelper(ev)});
		
		
		this.getRightPointer().style.right = '';
		this.initPointerPositions();
		Event.observe(window, 'resize', function(){obj.initPointerPositions()});
		Event.observe($$('.itoris_laynav .block-title')[0], 'click', function(){setTimeout(function(){obj.initPointerPositions()}, 100)});
	},

	touchHelper: function(ev) {
		ev.pageX = ev.touches[0] ? ev.touches[0].pageX : 0; ev.pageY = ev.touches[0] ? ev.touches[0].pageY : 0;
		if (ev.type.toLowerCase() == 'touchstart') {
			var ol = Element.cumulativeOffset(this.getLeftPointer());
			var or = Element.cumulativeOffset(this.getRightPointer());
		...
		if (ev.type.toLowerCase() == 'touchend') {var obj = this; setTimeout(function(){obj.onMouseUp(ev)}, 200);}
